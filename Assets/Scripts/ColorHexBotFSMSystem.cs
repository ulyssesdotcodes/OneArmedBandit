...
/// </summary>
public enum ColorHexTransition
{
...
/// </summary>
public class ColorHexBotFSMSystem
{
    private List<FSMState> states;
 
    // The only way one can change the state of the FSM is by performing a transition
    // Don't change the CurrentState directly
    private ColorHexStateID currentStateID;
    public ColorHexStateID CurrentStateID { get { return currentStateID; } }
    private FSMState currentState;
    public FSMState CurrentState { get { return currentState; } }
 
    public ColorHexBotFSMSystem()
    {
    ...
    /// </summary>
    public void DeleteState(ColorHexStateID id)
    {
    ...
    /// </summary>
    public void PerformTransition(ColorHexTransition trans)
    {
...
public class GoToHex : FSMState
{
	GameObject bot, targetHex;
	
	public GoToHex(GameObject bot, GameObject targetHex)
	{
		stateID = CollectorStateID.GoToResource;
		this.bot = bot;
		this.targetHex = targetHex;
	}
	
	public override void Reason (ColorHexBotFSMSystem fsm)
	{
		if(BotActions.Nearby(bot, targetHex))
		{
			fsm.PerformTransition(ColorHexTransition.ReachedHex);
		}
	}
	
	public override void Act ()
	{
		BotActions.MoveToDestination(targetHex);
	}
}

public class ChangeColor : FSMState
{
	long startTime;
	GameObject bot;
	Material newColor;
	
	public ChangeColor(Material newColor, GameObject bot)
	{
		stateID = ColorHexStateID.ChangeColor;
	...
	{
		//Check if it's waited 3 seconds at the resource
		if(DateTime.Now.Ticks - startTime > 3000)
